...
public final class Encryption
{
	...
	private Path createEncryptionFolder(Path encFolder) throws IOException
	{
		...
		String readmeText = "DO NOT SHARE THESE FILES WITH ANYONE!\r\n"
			...
			+ "If you do, Kalash's Alt Manager can no longer read your alt list and will replace it with a blank one.\r\n"
	...
	public static Path chooseEncryptionFolder()
	{
		...
		String encFolderName = ".Kalash encryption";
		String legacyEncFolderName = ".Wurst encryption";

		Path homeEncFolder = Paths.get(userHome, encFolderName).normalize();
		Path legacyHomeEncFolder =
			Paths.get(userHome, legacyEncFolderName).normalize();
		Path encFolder = homeEncFolder;
		if(xdgDataHome != null && !xdgDataHome.isEmpty())
			encFolder = Paths.get(xdgDataHome, encFolderName).normalize();

		Path legacyEncFolder = encFolder.resolveSibling(legacyEncFolderName);
		if(!Files.exists(encFolder))
		{
			if(Files.isDirectory(legacyEncFolder))
				migrateEncryptionFolder(legacyEncFolder, encFolder);
			else if(Files.isDirectory(legacyHomeEncFolder))
				migrateEncryptionFolder(legacyHomeEncFolder, encFolder);
		}
	...
	public static void migrateEncryptionFolder(Path oldFolder, Path newFolder)
	{
		...
		}catch(IOException e)
		{
			CrashReport report = CrashReport.forThrowable(e,
				"Migrating Kalash encryption folder");
